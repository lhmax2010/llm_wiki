...
def test_auth_context_rejects_missing_user_and_unknown_role(
    make_context: Callable[..., MiddlewareContext], roles_config: RolesConfig
) -> None:
    missing_user = make_context(include_roots=False)
    missing_user["auth"] = {"user": "", "role": "contributor"}
    unknown_role = make_context(include_roots=False)
    unknown_role["auth"] = {"user": "alice", "role": "ghost"}
    missing_result = auth_context(roles_config)(missing_user)
    unknown_result = auth_context(roles_config)(unknown_role)
    ...
    assert unknown_result["error"].field == "auth.role"
...
def test_evidence_validate_surfaces_core_errors(
    ...
    context = make_context(
        payload=entry_payload(evidence=[{"type": "human_note"}]),
    )
    context = schema_validate()(context)["context"]
...
def test_update_scope_rules_are_conservative(
    ...
    heavy = make_context(operation="update", payload=entry_payload(trust_state="published"))
    heavy["change_scopes"] = ["claim_type_change"]
    light = make_context(operation="update", payload=entry_payload(trust_state="published"))
    light["change_scopes"] = ["wording"]
    unknown = make_context(operation="update", payload=entry_payload(trust_state="published"))
...
def test_update_changed_fields_auto_light_and_heavy(
    ...
    auto = make_context(operation="update", payload=entry_payload(trust_state="published"))
    auto["changed_fields"] = ["tags"]
    light = make_context(operation="update", payload=entry_payload(trust_state="published"))
    light["changed_fields"] = ["aliases"]
    heavy = make_context(operation="update", payload=entry_payload(trust_state="published"))
    heavy["changed_fields"] = ["body"]
...
def test_review_route_sets_trust_state_from_target_dir(
    ...
    context = make_context(payload=entry_payload(trust_state="published"))
    context = schema_validate()(context)["context"]
...
def test_validation_failure_prevents_persist_and_audit(
    ...
    context = make_context(payload=entry_payload(evidence=[{"type": "human_note"}]))
    result = run_pipeline(
        context,
        [
            auth_context(roles_config),
