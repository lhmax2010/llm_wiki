...
Phase 4 是索引 + 检索层：实现 agent/human 两个真实索引、同义词、CJK bigram、`min_support` 段落穿透，并把 MCP `search_kb` 从 P3 直扫切到索引优先。R14 后按方向 B 明确：P4 的 SQLite 层是 validated path catalog，保证正确性/隔离/搜索质量；SQL/ripgrep 性能加速留后续。

## 本次范围

- 新增 `index/`：
  - `sqlite_index.py`：SQLite validated path catalog 与 rebuild。
...
- `kb/research/` 是否没有进入 SQLite path catalog。
...
- SQLite path catalog 是否可重建，坏 Markdown/坏 trust_state 是否容错跳过并记录日志。
- rebuild / fallback 是否都复用了 P1 `validate_entry` 三态一致性和 P3 resolve/is_relative_to 路径防护。
