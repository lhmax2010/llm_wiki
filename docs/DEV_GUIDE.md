...
Codex 会判这个 Phase 的风险档，你确认。**本项目风险档对照**（适配版）：
...
### 步骤 F：拍板放行（顺序别错）
你回"通过，可以 merge"后，Codex 按**固定顺序**收尾：
1. 前提：端到端验收过、review 闭环。
2. 写 dev_memory result（验收结果/遗留 TODO/决策小结）+ 更新 INDEX。
3. 提交进 PR 分支：`git add docs/dev_memory/... && git commit -m "[Phase N] docs: dev_memory result" && git push`
4. 然后 merge（有权限 + CI 过；否则报告阻塞不强合）。
5. 打 checkpoint tag（先切主分支再打）：
   ```bash
   git fetch origin && git switch main && git pull --ff-only
   git tag -a checkpoint/phase_N_<desc> -m "<PR号 + 验收摘要>"
   git push origin checkpoint/phase_N_<desc>
   ```
   - 高风险 phase 打 tag；低风险只在 checkpoints.md 记 commit hash。
