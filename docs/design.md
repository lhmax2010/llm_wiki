...
> **v1.3 变更（change_1，5 项）**：ID 统一 SQLite 发号 / CodeBinding 补字段 / 重建纳入 deprecated / observation 证据精确化 / P1 code_binding 只校验形状。
> **v1.4 变更（Phase 1 review change_2）**：明确正文段落规则——section_credibility 的 key 必须在骨架内（不在则打回），但**正文允许骨架外的额外补充 heading**（如"## 备注"），不打回。澄清 §3.4 段落骨架的"宽松"语义。
...
- 版本：v1.4
- 创建时间：2026-06-12（v1.2 FROZEN；v1.3 change_1；v1.4：2026-06-13，Phase 1 review change_2）
- 状态：**Frozen**
...
🟢 **段落 ↔ section_credibility 映射规则（v1.1，回应 Codex 实现歧义）**：正文用 Markdown heading（`## 根因`）作段落锚点，`section_credibility` 的 key 与 heading 文本对应（`section_credibility.根因` 覆盖"## 根因"段）。校验器按 heading 文本匹配。

🟢 **正文 heading 规则（v1.4，宽松语义）**：
- `section_credibility` 的 **key 必须在该 entry_type 的段落骨架内**，不在 → 打回（E_SCHEMA）。这是为了防止给不存在的段落标可信度。
- 但**正文本身允许出现骨架外的额外补充 heading**（如人随手加"## 备注"/"## 参考链接"），**不打回**。骨架定义的是"必须有的核心段落 + 可标可信度的段落"，不是"正文只能有这些段落"。
- 即：正文 heading ⊇ 骨架段落（可以多，核心段落不能少）；section_credibility key ⊆ 骨架段落（只能给骨架内段落标可信度）。
